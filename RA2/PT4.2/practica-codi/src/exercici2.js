...
async function main() {
    try {
        ...
        const llistajocs = games.slice(0,2);
        const sortida = {
            timestamp: Date.now(),
            games: []
        };

        for (const game of llistajocs) {
            let positive = 0;
            let negative = 0;
            let neutral = 0;
            let error = 0;

            const reviewsJoc = reviews
                .filter(r => String(r.app_id) === String(game.appid))
                .slice(0, 2);

            for (const review of reviewsJoc) {
                const sentiment = await analyzeSentiment(review.content);
            ...
            //console.log(`Sentiment (Ollama): ${sentiment}`);
            //console.log(`Sentiment (Positiu): ${positive}`);
            //console.log(`Sentiment (Error): ${error}`);
            //console.log(`Sentiment (Negatiu): ${negative}`);
            //console.log(`Sentiment (Neutral): ${neutral}`);
        ...
        fs.writeFileSync('exercici2-resposta.json', JSON.stringify(sortida, null, 2), 'utf-8');
